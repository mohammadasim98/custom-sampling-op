...
TF_SAMPLING_SRCS = tensorflow_sampling/cc/kernels/tf_sampling.cpp $(wildcard tensorflow_sampling/cc/ops/*.cpp)
...
TF_SAMPLING_GPU_ONLY_TARGET_LIB = tensorflow_sampling/python/ops/tf_sampling.cu.o
TF_SAMPLING_TARGET_LIB = tensorflow_sampling/python/ops/tf_sampling.so
...
# time_two op for GPU
tf_sampling_gpu_only: $(TF_SAMPLING_GPU_ONLY_TARGET_LIB)

$(TF_SAMPLING_GPU_ONLY_TARGET_LIB): tensorflow_sampling/cc/kernels/tf_sampling.cu
	$(NVCC) -std=c++11 -c -o $@ $^  $(TF_CFLAGS) -D GOOGLE_CUDA=1 -x cu -Xcompiler -fPIC -DNDEBUG --expt-relaxed-constexpr

tf_sampling: $(TF_SAMPLING_TARGET_LIB)
$(TF_SAMPLING_TARGET_LIB): $(TF_SAMPLING_SRCS) $(TF_SAMPLING_GPU_ONLY_TARGET_LIB)
...
clean:
	rm -f $(ZERO_OUT_TARGET_LIB) $(TF_SAMPLING_GPU_ONLY_TARGET_LIB) $(TF_SAMPLING_TARGET_LIB)
